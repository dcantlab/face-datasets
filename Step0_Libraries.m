...
if ~strncmp(computer('arch'), 'maca', 4)  % Workaround for Apple silicon macs
	...
	if ~exist(fullfile(libRoot, matConvNet.dir), 'dir')
		...
		if any(arrayfun(@(v) strcmp(v.Name, 'Parallel Computing Toolbox'), ver))
			newScript = strrep(orgScript, 'toolboxdir(''distcomp'')', 'toolboxdir(''parallel'')');
		else
			newScript = strrep(orgScript, 'toolboxdir(''distcomp'')', '''compatibility'',''parallel''');
		end
		fid = fopen(newFile, 'w');
...
if ~exist(fullfile(libRoot, openFace.dir), 'dir')
	...
	% Workaround for Apple silicon macs
	if strncmp(computer('arch'), 'maca', 4)
		orgFile = fullfile(libRoot, openFace.dir, 'matlab_version', 'face_detection', 'mtcnn', 'detect_face_mtcnn.org.m');
		newFile = fullfile(libRoot, openFace.dir, 'matlab_version', 'face_detection', 'mtcnn', 'detect_face_mtcnn.m');
		movefile(newFile, orgFile);
		orgScript = fileread(orgFile);
		newScript = strrep(orgScript, ...
			'fprintf(''Warning MatConvNet is not installed or not setup', ...
			'%fprintf(''Warning MatConvNet is not installed or not setup');
		fid = fopen(newFile, 'w');
		fwrite(fid, newScript);
		fclose(fid);
...
fprintf(' (4) pipelines%ssample-pipeline%s\n\n', filesep, filesep);
